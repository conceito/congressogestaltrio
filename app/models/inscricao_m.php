...
//		if($this->isDebug())
//		{
//			d($this->getInscriptionComment($data));
//			dd($this->input->post());
//		}
		...
		$inscriptionId = $this->cms_usuario->inscribe(array(
			...
			'comentario'  => $this->getInscriptionComment($data),
			'user_id'     => $userId,
			'status'      => 2
		), false, true);
	...
	private function getInscriptionComment($postData)
	{
		if (isset($postData['tipo_usuario'])
			&& ($postData['tipo_usuario'] == 'profissional_grupo'
				|| $postData['tipo_usuario'] == 'especializacao_grupo')
		)
		{
			return "Coordenador: {$postData['nome_coord']} {$postData['email_coord']}";
		}
	...
	private function saveUserMetadata($userId, $data)
	{
		...
		$this->cms_metadados->saveByUser($userId, array(
			'meta_key'   => 'nome_coord',
			'meta_type'  => '',
			'meta_value' => $data['nome_coord']
		));
		$this->cms_metadados->saveByUser($userId, array(
			'meta_key'   => 'email_coord',
			'meta_type'  => '',
			'meta_value' => $data['email_coord']
		));
		$this->cms_metadados->saveByUser($userId, array(
