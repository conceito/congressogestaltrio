...
class Trabalhos_model extends MY_Model
{
	...
	public function allByAuthor($authorId, $options = array())
	{
		...
		if ($qAuthorJobs->num_rows() == 0)
		{
			return null;
		}

		return $this->decorateCollection($qAuthorJobs->result_array(), $options);
	}
	...
	private function decorateCollection($array, $options = array())
	{
		...
		{
			$collection[] = $this->decorateJob($item, $options);
		}
	...
	/**
	 * decorate the job item
	 *
	 * @param $item
	 * @param array $options
	 * @return mixed
	 */
	public function decorateJob($item, $options = array())
	{
		if (isset($item['dt_ini']))
		{
			$item['dt_ini'] = formaPadrao($item['dt_ini']);
		}
		if (isset($item['dt_fim']))
		{
			$item['dt_fim'] = formaPadrao($item['dt_fim']);
		}

		if (isset($item['titulo']))
		{
			$item['titulo_txt'] = clean_html_field($item['titulo']);
		}
		...
		$metas = $this->cms_metadados->getAllByContent($item['id']);

		// subtítulo
		$item['subtitulo'] = get_meta($metas, 'subtitulo', null, true);

		// eixo temático
		$tema                   = $this->congresso->getTema(get_meta($metas, 'eixo_tematico', null, true));
		$item['eixo_tematico'] = $tema['title'];

		// modalidade
		$moda                = $this->congresso->getModalidade(get_meta($metas, 'modalidade', null, true));
		$item['modalidade'] = $moda['title'];

		// authors
		$authors                = get_meta($metas, 'authors', null, true);
		$item['autores'] = unserialize($authors);
