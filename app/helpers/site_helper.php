...
if(! function_exists('err'))
{
    function err($field = null){
        echo (form_error($field)) ? 'invalid' : '';
    }
}

if(! function_exists('next_valid_pay'))
{
    /**
     * receive the pay day and return the next (closest) pay day
     *
     * @param $venc yyyy-mm-dd
     * @return bool|string yyyy-mm-dd
     */
    function next_valid_pay($venc)
    {
        $aMonth = voltaData(formaPadrao($venc), 30);
        $isFarAway = ($aMonth > date("Y-m-d")) ? true : false;
        $nextMonth = date("m") + 1;
        $nextMonth = substr("0" . $nextMonth, -2);
        $newVenc = (date("Y-m-10") <= date("Y-m-d")) ? date("Y-{$nextMonth}-10") : date("Y-m-10");
        return $newVenc;
    }
}
...
//        * "tags"   => target
        ...
        if ($sql->num_rows() > 0) {
            ...
            $saida .= '<ul id="main-menu" ' . $classId . '>' . PHP_EOL;
            ...
            for ($x = 0; $x < count($nivel_0); $x++) {
                ...
                $title = $nivel_0[$x]['txt'];
                $css = $nivel_0[$x]['resumo'] . ($hasChildren) ? ' -dropdown-submenu' : '';
                ...
                for ($j = 0; $j < count($itens); $j++) {
                    ...
                    $title_1 = $itens[$j]['txt'];
                    $css_1 = $itens[$j]['resumo'] . ($hasChildren) ? ' -dropdown-submenu' : '';
                    ...
                    if ($pai_id_1 == $item_id) {
                        ...
                        for ($u = 0; $u < count($itens); $u++) {
                            ...
                            $title_2 = $itens[$u]['txt'];
                            $css_2 = $itens[$u]['resumo'] . ($hasChildren) ? ' -dropdown-submenu' : '';
